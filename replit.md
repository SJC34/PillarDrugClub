...
- **HubSpot CRM Integration** (March 2026): Uses Replit's HubSpot connector (OAuth). `server/hubspot.ts` exports `syncUserToHubSpot()` which upserts contacts by email with `lifecyclestage` and `subscription_status` properties. Sync hooks: registration → `lead`, subscription activation → `customer/active`, cancellation → `canceled`. All HubSpot calls are fire-and-forget (non-blocking).
