...
**Medication Search System**: Dual-API integration providing comprehensive medication search and catalog functionality. **RxNorm API** (National Library of Medicine) powers real-time autocomplete across all medication search inputs site-wide, delivering deduped medication suggestions with rxcui identifiers after 2+ character input with 300ms debounce. **DailyMed API** (FDA's official drug database) serves as the authoritative medication catalog, automatically importing 34+ FDA-approved medications on startup including Lisinopril, Metformin, Atorvastatin, Gabapentin, Sertraline, and other common prescriptions. The MedicationSearch component (client/src/components/MedicationSearch.tsx) features autocomplete dropdown with keyboard navigation, clear functionality, and loading indicators, integrated across PrescriptionTransferPage, MedicationsPage, and CostCalculatorPage. Backend endpoints: GET /api/medications/search?q={query} for RxNorm autocomplete, GET /api/medications for DailyMed catalog with pricing. Import system uses setid-based deduplication, category-aware pricing, and 200ms request throttling to respect API limits. Storage automatically loads DailyMed medications during startup sequence (importDataOnStartup → loadImportedMedications), replacing the legacy Excel import system.
