...
**Medication Search System**: Dual-API integration providing comprehensive medication search and catalog functionality. **RxNorm API** (National Library of Medicine) powers real-time autocomplete across all medication search inputs site-wide, delivering deduped medication suggestions with rxcui identifiers after 2+ character input with 300ms debounce. **SJC Pharmacy CSV** serves as the authoritative medication catalog, automatically importing 1857+ medications on startup with real wholesale pharmacy pricing covering Abilify, Acyclovir, Albuterol, Alprazolam, Amoxicillin, Atorvastatin, and a comprehensive range of prescription medications. The MedicationSearch component (client/src/components/MedicationSearch.tsx) features autocomplete dropdown with keyboard navigation, clear functionality, and loading indicators, integrated across PrescriptionTransferPage, MedicationsPage, and CostCalculatorPage. Backend endpoints: GET /api/medications/search?q={query} for RxNorm autocomplete, GET /api/medications for pharmacy catalog with pricing. CSV parser (scripts/import-pharmacy-csv.ts) extracts drug names, strengths, dosage forms, and pricing; wholesale price calculated as 20% of unit cost. Storage automatically loads CSV medications during startup sequence (importDataOnStartup → loadImportedMedications → importMedicationsFromCSV).
