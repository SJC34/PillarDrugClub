...
- **Authentication & Authorization**: Dual authentication via email/password and Google OAuth, using Passport.js for session management and a robust role-based access control system. **Persistent Login System** (November 2025): Production-ready offline-first authentication with localStorage-based session persistence that survives browser restarts, graceful handling of transient server errors (5xx) and 304 "Not Modified" responses without logout, race condition fixes ensuring navigation waits for React state updates, and comprehensive debug logging for production monitoring.
