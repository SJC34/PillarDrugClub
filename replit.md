...
**Medication Search System**: CSV-based medication search and catalog functionality using the SJC Pharmacy pricing file as the sole data source. The system automatically imports 1857+ medications on startup with real wholesale pharmacy pricing covering Abilify, Acyclovir, Albuterol, Alprazolam, Amoxicillin, Atorvastatin, and a comprehensive range of prescription medications. The MedicationSearch component (client/src/components/MedicationSearch.tsx) features autocomplete dropdown with keyboard navigation, clear functionality, and loading indicators, integrated across PrescriptionTransferPage, MedicationsPage, and CostCalculatorPage. Backend endpoint GET /api/medications/search?q={query} searches the local CSV catalog with 300ms debounce, returning medications matching the query by name, generic name, brand name, or category. CSV parser (scripts/import-pharmacy-csv.ts) extracts drug names, strengths, dosage forms, and pricing; wholesale price calculated as 20% of unit cost. Storage automatically loads CSV medications during startup sequence (importDataOnStartup → loadImportedMedications → importMedicationsFromCSV).
